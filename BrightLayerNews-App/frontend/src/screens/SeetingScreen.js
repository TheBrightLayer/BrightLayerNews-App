...
import SideMenu from "../components/SideMenu"; // ✅ added like HomeScreen
...
export default function SettingsScreen({ navigation }) {
  ...
  const [menuOpen, setMenuOpen] = useState(false); // ✅ same as HomeScreen
  ...
  // ✅ Side menu navigation handling (same style as HomeScreen)
  function onNavigateFromMenu(id) {
    if (id === "dark") {
      toggleTheme();
    } else if (id === "settings") {
      // already on settings, just close
      setMenuOpen(false);
      ...
      >
        ...
        >
          <View className="flex-row justify-between items-center">
            <View>
              <Text
                className={`${theme.colors.text} text-base font-semibold`}
              >
                Dark Mode
              </Text>
        ...
        >
          <Text
            className={`${theme.colors.text} text-base font-semibold mb-2`}
          >
            Font size
          </Text>
